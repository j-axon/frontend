# ISSUE-FE-013 — Configurar Playwright para pruebas E2E críticas
...
Implementar **Configurar Playwright para pruebas E2E críticas** sin romper el contrato con Backend. El refresh token nunca debe leerse ni guardarse en frontend; viaja por cookie HttpOnly enviada automáticamente con `credentials: 'include'`.
...
|---:|---|---|---|
| 1 | Config | `playwright.config.ts` | `Playwright config` |
| 2 | E2E | `tests/e2e/auth-login.spec.ts` | `AuthLoginE2E` |
| 3 | E2E | `tests/e2e/qr-ticket.spec.ts` | `QrTicketE2E` |
| 4 | E2E | `tests/e2e/technician-flow.spec.ts` | `TechnicianFlowE2E` |
| 5 | E2E | `tests/e2e/auditor-readonly.spec.ts` | `AuditorReadonlyE2E` |
| 6 | Fixtures | `tests/e2e/fixtures/users.ts` | `users fixture` |
| 7 | Helpers | `tests/e2e/helpers/auth.ts` | `auth helper` |
...
- `e2e/auth-login-logout.spec.ts`: login/logout.
- `e2e/qr-to-ticket.spec.ts`: flujo crítico QR ticket.
- `e2e/rbac-navigation.spec.ts`: navegación por rol.

### Tests de accesibilidad

- `Playwright axe opcional`: validar páginas críticas.
