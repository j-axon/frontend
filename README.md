...
A grandes rasgos, las pantallas y secciones que verás al levantar `pnpm run dev`:
...
> **Pre-requisito**: el backend tiene que estar corriendo en `http://localhost:8080`
> (ver [`backend/README.md`](../backend/README.md)). El front hace fetch contra
> `NEXT_PUBLIC_API_BASE_URL` (default `http://localhost:8080/api/v1`) y se conecta al
> WebSocket en `NEXT_PUBLIC_WS_URL` (default `ws://localhost:8080/ws`).

```bash
# 1) Variables de entorno
cp .env.example .env.local

# 2) Instalar dependencias (pnpm únicamente — nunca npm)
pnpm install
...
> `pnpm` es el package manager exclusivo de este proyecto. No usar `npm` ni `yarn`
> (incluso para instalaciones ad-hoc). Si tenés `npm install` en un script, va a romper.
...
pnpm run test
pnpm run build
npx playwright test    # equivalente: pnpm run test:e2e    # si la issue toca flujos críticos
```
